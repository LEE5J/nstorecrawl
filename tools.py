import logging, sys, os, requests, time
import traceback
...
# get_categoryid_byname('수도관 주방 욕실 에어컨 배관 싱크대 누수 방수 테이프')
...
def convert_to_frame(product, prefix, index,jpg_pathes):
    ...
    line.append(product.seller_code)  # 판매자 상품코드
    line.append("")  # 판매자 바코드
...
def export_option(option_list):
    productcode = int(time.time()*100)
    seller_productcode.append(productcode)
    if len(option_list[0]) == 3: # 2중 옵션인 경우에 해당한다.
        data = []
        for option in option_list:
            data.append([option[0], option[1], option[2], 9999, "", "Y"])
        pd.DataFrame(data).to_excel(f"{productcode}.xlsx", encoding='cp949', index=False, header=["선택1", "선택2", "옵션가", "재고수량", "관리코드", "사용여부"])
    else:
        data = []
        for option in option_list:
            data.append([option[0], option[1], option[2], option[3], 9999, "", "Y"])
        pd.DataFrame(data).to_excel(f"{productcode}.xlsx", encoding='cp949', index=False, header=["선택1", "선택2", "선택3", "옵션가", "재고수량", "관리코드", "사용여부"])
    return productcode
...
def trim_html(html):
    start_index = 0
    while True:
        current_index = html.find("<img data-src", start_index)
        if current_index == -1:
            break
        cutpoint1 = current_index + 4 # ~9 까지 짜르기
        cutpoint2 = html.find(" src=\"", start_index + 10)
        cutpoint3 = html.find("\"", cutpoint2 + 8)
        html = html[0:cutpoint1+1] + html[cutpoint1+6:cutpoint2] + html[cutpoint3+1:]
        start_index = cutpoint2
    return html
...
class Product:
    ...
    def print_all(self):
        print(self.product_name, self.saled_price, self.option_title_list, self.option_name_list, self.addopt_name_list, self.detail_html)